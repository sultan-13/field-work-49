...
identical_matrix = [[1, 0, 0], [0, 1, 0], [0, 0, 1]]
print("identical matrix = ", identical_matrix)


# accessing
first_elem = identical_matrix[1][1]
print(first_elem)

# modifying
identical_matrix.append([1, 1, 1])   # appending a row
print(identical_matrix)

identical_matrix.remove([1, 1, 1])  # removing a certain row
print(identical_matrix)

identical_matrix.pop()
print(identical_matrix)

identical_matrix.insert(1, [1, 1, 1]) # inserting in a certain position
print(identical_matrix)
...
std_details = ("Miskat Raihan", 23, 3.7, True)

std_dict = {
    "std_name" : std_details[0],
    "std_age"  : std_details[1],
    "std_cgpa" : std_details[2],
    "is_engr" : std_details[3]
}

for xx in std_dict:    # only value will be printed
    print(std_dict[xx])

for yy, zz in std_dict.items():    # both key & value will be printed
    print(yy, " : ", zz)

# print(std_dict["std_name"])
...
grade_list = [("Miskat", 3.7), ("Sultan", 3.9), ("Emon", 3.85), ("Manik", 2.97)]

sorted_gradelist = sorted(grade_list, key= lambda x:x[1])
print(sorted_gradelist)

it = len(sorted_gradelist);
for rank in sorted_gradelist:
    print("rank ", it , " = ", rank)
    it -= 1
