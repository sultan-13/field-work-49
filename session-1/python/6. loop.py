...
temperatures = [39, 28, 35, 27, 33, 29]
threshold = 32

for temp in temperatures:
    if temp > threshold:
        print("Warning: Temperature", temp, "is above the threshold of", threshold)
...
users = [{"email": "miskat@gmail.com", "subscribed": True},
         {"email": "raihan@hotmail.com", "subscribed": False},
         {"email": "mr9@yahoo.com", "subscribed": False}]
for user in users:
    if user["subscribed"]:
...
products = {
    "RAM": {"stock": 16, "minimum_required": 20},
    "SSD": {"stock": 15, "minimum_required": 12}
}

for product, details in products.items():
    if details["stock"] < details["minimum_required"]:
        print(f"Empty Alert: {product} stock is low. Restock, ASAP.")
    else:
        print(f"{product} stock is enough.")
